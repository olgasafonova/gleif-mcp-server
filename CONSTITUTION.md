...
**Standing violations, named rather than hidden.** The version constants have drifted from the tags: the latest tag is `v0.9.0`, while `ServerVersion` in `main.go:21` still reads `0.7.0` (so a released binary self-reports the wrong version over MCP) and the `User-Agent` at `internal/gleif/client.go:193` is frozen at `0.2.0`. The tracked `server.json` and `manifest.json` carry `0.5.0`, which is inert at release time because `release.yml` regenerates or overrides them, but the two constants ship in the binary. Fixing them is a code change, not an amendment; this article records the gap so it cannot be mistaken for policy.
